...
function calcVRVPipePerFt(
  ...
  const weights = PIPE_WEIGHTS[pipeSize];
  ...
  const baselineWeightL = PIPE_WEIGHTS[pipeSize]['L'];     // calibration ref
  ...
  const baselinePricePerFt = (VRV_BASELINE_PRICES_PER_FT[pipeSize] || 0) * typePremiumFactor;
  ...
  const insulationPerFt = includeInsulation
    ? (INSULATION_COSTS_PER_FT[pipeSize] || 0)
    : 0;
...
function calcVRVBlendedPerFt(
  ...
  const cfg = VRV_BLENDED_CONFIGS[tonnage];
...
function calcSplitCost(
  ...
  const basePrices = BASELINE_PRICES[equipType];
  ...
  const pipeConfig = EQUIPMENT_PIPE_CONFIGS[equipType]?.[tonnage];
...
function calcAHUCost(
  ...
  const row = BASELINE_PRICES.ahuWithCU[tonnage];
  ...
  const pipeConfig = EQUIPMENT_PIPE_CONFIGS.ahuWithCU?.[tonnage];
...
function calcRefrigerantCharge(equipType, tonnage, isLongRun) {
  const row = BASELINE_PRICES.refrigerant?.[equipType]?.[tonnage];
  if (!row) return null;
...
function generateVRVPipeSizeTable(
  ...
  const table = {};
  Object.keys(VRV_BASELINE_PRICES_PER_FT).forEach(size => {
...
module.exports = {
  ...
  // ── Reference data (for frontend dropdowns / display) ─────
